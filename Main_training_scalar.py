...
train = True
reverse = False
sample_classes = None #np.array([0, 4])
checkpoint = ''
cycle_sample = None
random = False

transfert = None
transfert_date = '210713'
transfert_pred = 'v10'
transfert_sub_version = ''
transfert_trainsize = 1000000
...
PARAMS = dict3 = {'date': args.d, 'num_model': model, **version_exp, **model_spec}
neptune.create_experiment('Run_{}'.format(num_test), params=PARAMS)
...
if args.transfert is not None:
    ...
    model, optimizer = def_nn.recup_from_checkpoint(args, model, optimizer, num_test, load_best_acc, load_best_loss,
                                                    checkpoint)
...
if config_pred.output_type == 'class':
    ...
    plot.fioritures(ax, fig, title='renorm count sur {}'.format(ysave) if title else None, label=None, grid=grid,
                    save=save)
    ...
    for i in range(nbclasses):
        for k in range(nbclasses):
            ax.plot(X_conf[k], (conf[i, k] / norm[k]) * 100, color=colorpred[i], marker='.', linestyle="None",
                    label='classe {}'.format(i))
            plot.plt.plot([X_conf[k] - 0.5, X_conf[k] + 0.5],
                          [(conf[i, k] / norm[k]) * 100, (conf[i, k] / norm[k]) * 100], color=colorpred[i])
    # for i in range(nbclasses + 1):

...
parser.add_argument("--model", type=str, default="1_2_3")
parser.add_argument("--gpu", type=int, default=0)
parser.add_argument("--max-nb-episode", type=int, default=75)
parser.add_argument("--max-nb-eval", type=int, default=50)

parser.add_argument("--replay-start-size", type=int, default=50000)
parser.add_argument("--target-update-interval", type=int, default=2500)
...
if not args.raw:
    ...
    ENV_NAME_EVAL = 'env-KnitCity-eval-v{}'.format(args.env)
else:
    ENV_NAME = 'env-KnitCityRaw-v{}'.format(args.env)
    ENV_NAME_EVAL = 'env-KnitCityRaw-eval-v{}'.format(args.env)
...
args.outdir = global_path_save + "Knit_City/results{}/{}/{}/env_v{}".format(
    args.results, args.sub_set, env.lab.version_pred if not args.raw else '{}_raw'.format(env.lab.version_pred), args.env)
...
args.steps = env.lab.indexes.shape[
                 0] * env.lab.nb_step if args.max_nb_episode is None else np.min(np.array([args.max_nb_episode * env.lab.nb_step,
                                                                                 env.lab.starts_episodes.shape[
                                                                                     0] * env.lab.nb_step]))
args.eval_interval = 2 * env.lab.nb_step
...
R_mean = np.zeros(nb_test)
R_delphes_mean = np.zeros(nb_test)
R_simplet_mean = np.zeros(nb_test)
out_days = np.zeros(nb_test)
for i in range(nb_test):
    R_mean[i] = np.mean(R_test[i, :])
    R_delphes_mean[i] = np.mean(R_delphes[i, :])
    R_simplet_mean[i] = np.mean(R_simplet[i, :])
    out_days[i] = np.sum(out_days_test[i, :, :])

best = np.where(R_mean == np.max(R_mean))

print('RL R mean {}'.format(R_mean))
print('with days out : {}'.format(out_days))
print('best on {} of reward {}'.format(best, R_mean[best]))
for i in range(5):
    print('out decade {} : {}'.format(i+1, np.mean(out_days_test[best, :, i])))

print('vs delphes : {} | simplet {}'.format(R_delphes_mean, R_simplet_mean))

...
class KnitLab():
    def __init__(self, date, version_pred, model, trainsize, sub_version,
        ...
        self.config_data = ConfigData(Config_data.exp['knit005_mix_v22'])
    ...
    def set_lab_args(self, nb_eval, info_size, model_set, RL_set):
        ...
        signal_flu = SignalForce(self.config_data, 'flu_rsc', self.model_set)
        signalevent = ForceEvent(self.config_data, signal_flu.f, signal_flu.ext, signal_flu.t,
        ...
        self.knowledge = np.ones(info_size) * np.NaN
        self.god_knowledge = np.ones(info_size) * np.NaN
        self.df_now = None
        self.df_next = None
    ...
    def update_knowledge(self, index):
        if self.info_size == 1:
            self.knowledge = np.array([int(self.y_pred[index])]).astype(int) if self.y_pred is not None else None
            self.god_knowledge = np.array([self.get_class_label(self.y_value[index])]).astype(int)
        else:
            self.knowledge = self.y_pred[int(index - self.info_size + 1): int(index + 1)].astype(
                int) if self.y_pred is not None else None
            self.god_knowledge = np.array([self.get_class_label(self.y_value[i]) for i in
...
class DecisionalKnitCity(gym.Env):
    """
    ...
    def __init__(self,  city, lab, simplet, delphes, lab_args,  decade, dt, knowledge_type,
        ...
        self.low_state = np.array([0 for _ in range(self.knowledge.size)]).astype(np.int)
        self.high_state = np.array([self.nbclasses for _ in range(self.knowledge.size)]).astype(
            np.int)  ### test contre 1 avant

        self.action_space = spaces.Discrete(2)
        self.observation_space = spaces.Box(low=self.low_state, high=self.high_state, dtype=np.int)
        self.state = None
    ...
    def step(self, action):
        ...
        if self.knowledge_type == 'NN':
            action_simplet = decision(self.simplet, int(self.lab.knowledge[-1]))
        else:
            action_simplet = decision(self.simplet, int(self.lab.god_knowledge[-1]))
